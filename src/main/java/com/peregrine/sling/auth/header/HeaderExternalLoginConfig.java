...
public @interface HeaderExternalLoginConfig
{
    ...
    @AttributeDefinition(name="Sync Handler Name", description="Name of the sync handler to be retrieved from the SyncManager")
    String sync_handlerName() default "default"; // OSGi property: sync.handlerName

    @AttributeDefinition(name="External IDP Name", description="Name of the external IDP to be retrieved from the ExternalIdentityProviderManager")
    String idp_name(); // OSGi property: idp.name
}
